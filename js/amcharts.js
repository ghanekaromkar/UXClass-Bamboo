        ...
        parseDatesDraw: function() {
            ...
            if (0 < this.gridCountR)
                for (a = k; a <= B; a++) {
                    ...
                    m = l = p;
                    if (!isNaN(u))
                        for (h = 1; h < v; h += u) this.gridAlpha = this.minorGridAlpha, f = p + y * h, f = d.resetDateToMin(new Date(f), w, u, t).getTime(), f = new this.axisItemRenderer(this, (f - this.startTime) * this.stepWidth), this.pushAxisItem(f);
                    this.gridAlpha = z
                }
        },
                    ...
                    if (c.mouseIsOver || B || D || this.forceShow)
                        if (this.forceShow = !1, D) {
                            ...
                            0 !== h && (k = {
                                type: "zoomed",
                                target: this
                            }, k.chart = this.chart, g.parseDates && !g.equalSpacing ? (c + h > w && (h = w - c), B + h < v && (h = v - B), k.start = Math.round(B + h), k.end = Math.round(c + h), this.fire(k.type, k)) : J + h >= this.data.length || 0 > m + h || (k.start =
                                        ...
                                        if (y = C[g], w = NaN, (!this.oneBalloonOnly || y == J) && y.showBalloon && !y.hidden && y.balloonText && ("step" == y.type && "left" == y.stepDirection && (t = this.data[a + 1]), t)) {
                                            ...
                                            if (this.showNextAvailable && isNaN(w) && a + 1 < this.data.length)
                                                for (z = a + 1; z < this.data.length; z++)
                                                    if (e = this.data[z])
                                                        if (v = e.axes[y.valueAxis.id].graphs[y.id], w = v.y, !isNaN(w)) break;
                                            if (!isNaN(w)) {
                                                e = v.x;
                                                l = !0;
                                                if (u) {
